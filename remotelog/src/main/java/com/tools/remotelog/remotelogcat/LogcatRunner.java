...
import com.tools.remotelog.server.WebSocket;
import com.tools.remotelog.server.WebSocketServer;
...
public class LogcatRunner{
    ...
    private void init() throws IOException {
                ...
                if (mCurrWebSocket != null) {
                    ...
                    mCurrWebSocket.setWebSocketCallback(new WebSocket.WebSocketCallback() {
                        ...
                        public void onReceivedFrame(byte[] bytes) {
                            RemoteLogTool.Companion.log("onReceivedFrame --> " + new String(bytes));
                        }

                        @Override
                        public void onClosed() {
                            RemoteLogTool.Companion.log("onClosed --> " + mCurrWebSocket);
                            mCurrWebSocket = null;
                        }
                    });
                }
        ...
        mProcessOutputCallback = this::sendText;
    }
            ...
            try {
                ...
                try {
                    if (logConfig.write2File) {
                        File f = getLogFile();
                        RemoteLogTool.Companion.log("--> write to file " + f);
